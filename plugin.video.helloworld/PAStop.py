...
if str(Astop)=="Yes":
	...
	writefile = open(LF, 'w')
	writefile.write("mort")
	writefile.close()
	xbmc.executebuiltin('Notification(HOMIXBMC Notification,AudioPlayer STOP,5000,'+icon_path+')')
		...
		for it in range(len(strtuple)):
			...
			if strtuple[it].find("_Name")==1:
				if str(strtuple[it+1][:-1])==str(device_instance_name):
					...
					client.service.ChangeValueOfDeviceSimple(IDServer,IDDeviceXBMC,"PlayerAStop" )