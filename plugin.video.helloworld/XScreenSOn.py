...
if str(xssON)=="Yes":
	xbmc.executebuiltin('Notification(HOMIXBMC Notification,ScreenSaver START,5000,'+icon_path+')')
		...
		for it in range(len(strtuple)):
			...
			if strtuple[it].find("_Name")==1:
				if str(strtuple[it+1][:-1])==device_instance_name:
					...
					client.service.ChangeValueOfDeviceSimple(IDServer,IDDeviceXBMC,"XssON" )
