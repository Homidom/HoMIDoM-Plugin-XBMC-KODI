...
if str(Astart)=="Yes":
		print device_instance_name 
		LF = cwd_path+"\\temphomixbmc.txt"
		readfile = open(LF, 'r')
		for line in readfile:
		  	
		
			print "attnetttionrrrr"
			print line [1]
		
			if line[1]=="o":
				xbmc.executebuiltin('Notification(HOMIXBMC Notification,AudioPlayer PLAY,5000,'+icon_path+')')
				...
				for devices in Alldevices[0]:
					...
					for it in range(len(strtuple)):
						...
						if strtuple[it].find("_Name")==1:
							if str(strtuple[it+1][:-1])==device_instance_name:
								ValueDeviceXBMC= strtuple[it+19][:-1]
								IDDeviceXBMC= str(strtuple[it-45][:-1])
								client.service.ChangeValueOfDeviceSimple(IDServer,IDDeviceXBMC,"PlayerAStart" )
					#xbmc.executebuiltin('Notification(HOMIXBMC,This is a simple example of notifications,5000,'+icon_path+')')		
