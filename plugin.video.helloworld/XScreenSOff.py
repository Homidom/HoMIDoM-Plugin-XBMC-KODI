...
xssON=__addon__.getSetting("xbmc_screenSOFF")
XssOFF=__addon__.getSetting("xbmc_screenSOFF") 
...
if str(XssOFF)=="Yes":
	xbmc.executebuiltin('Notification(HOMIXBMC Notification,ScreenSaver STOP,5000,'+icon_path+')')
		...
		for it in range(len(strtuple)):
			...
			if strtuple[it].find("_Name")==1:
				if str(strtuple[it+1][:-1])==device_instance_name:
					...
					client.service.ChangeValueOfDeviceSimple(IDServer,IDDeviceXBMC,"XssOFF" )
